%% ------------------------------------------------------------
%  EMS with PV, Wind, BESS solved by quadprog (interior-point-convex)
%  Variables order (NO PV/Wind in x now):
%     x = [Pbuy(1:T); Psell(1:T); Pch(1:T); Pdis(1:T); E(1:T)]
%  Objective: min sum_t (pi_buy*Pbuy - pi_sell*Psell + c_sto*(Pch+Pdis)) + (ε/2)*||x||^2
...
%% ---------------- Objective: f and small quadratic term H ----------------
f = zeros(n,1);
f(ixPbuy)  = pi_buy;
f(ixPsell) = -pi_sell;
f(ixPch)   = c_sto;
...
% very small ε makes the problem strictly convex but keeps LP objective nearly unchanged
eps_qp = 1e-6 * max([pi_buy, c_sto, 1]);      % scale-aware tiny regularization
H = speye(n) * eps_qp;                         % H ≻ 0
% 如果想更平滑 Pbuy/Psell/Pch/Pdis 而少惩罚 SoC，可用：
% w = ones(n,1) * eps_qp; w(ixE) = eps_qp*0.1; H = spdiags(w,0,n,n);
...
% (1) Power balance: Pbuy - Psell + Pdis - Pch = Load - PV - W
for t = 1:T
    row = row + 1;
    Aeq(row, ixPbuy(t))  =  1;
    Aeq(row, ixPsell(t)) = -1;
...
t_total = tic;
% x0 可省略, interior-point-convex 不强制需要初值
[x, fval, exitflag, output] = quadprog(H, f, A, b, Aeq, beq, lb, ub, [], opts);
total_time = toc(t_total);

assert(exitflag > 0, 'quadprog failed: %s', output.message);
...
Ppv_v   = PV_avail;   % must-take
Pwt_v   = W_avail;

cost_v  = fval;
fprintf('Optimal operating cost (with tiny quadratic): %.6f $\n', cost_v);
fprintf('Total time: %.6f s\n', total_time);

%% ---------------- Plots ----------------
t = 1:T;

% Figure 1: PV, Wind, Load
...
% Figure 3: SoC and Charge/Discharge
figure('Color','w'); hold on; box on;
yyaxis right
...
title('BESS SoC and Charge or Discharge');
legend([pSoC  bDis bCha ], {'SoC','Discharge','Charge'}, ...
       'Location','northoutside','Orientation','horizontal');
grid on; set(gca,'FontName','Times New Roman','FontSize',12);
