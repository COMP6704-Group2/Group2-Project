%% ============= 用 quadprog 的 active-set 求解 强凸 QP =============
clear; clc;

%% ---------------- Switch ----------------
QP_ALG = 'active-set';   % 使用活跃集法
...
% 极小正则, 保证 H 正定同时尽量不改变 LP 目标值
eps_qp = 1e-6 * max([pi_buy, c_sto, 1]);   % 根据成本量级缩放
H = speye(n) * eps_qp;                     % 严格凸
% 若希望对 SoC 惩罚更小可用:
...
%% ---------------- 构造可行初值 x0（active-set 必需） ----------------
Pnet = Load - PV_avail - W_avail;   % 净负荷, 正表示购电, 负表示上网
Pbuy0  = max(Pnet, 0);
Psell0 = max(-Pnet, 0);
Pch0   = zeros(T,1);
Pdis0  = zeros(T,1);
...
% 与上下界对齐
Pbuy0  = min(Pbuy0,  ub(ixPbuy));
Psell0 = min(Psell0, ub(ixPsell));
E0path = min(max(E0path, lb(ixE)), ub(ixE));
...
%% ---------------- Unpack solution ----------------
ix = struct('buy',ixPbuy,'sell',ixPsell,'ch',ixPch,'dis',ixPdis,'E',ixE);
Pbuy_v  = x(ix.buy);
Psell_v = x(ix.sell);
Pch_v   = x(ix.ch);
Pdis_v  = x(ix.dis);
